...
1. Run `python -B <skill-root>/scripts/audit_documentation.py .` from the target repository root for new docs, large rewrites, doc moves, missing-doc investigations, or unclear structure. Add `--json` for the stable `{error, ok, result}` envelope. The helper reports only root-relative source paths and link ordinals, validates UTF-8 and notebook shapes, and reads each file once through a bounded no-follow handle with race checks. It rejects local hardlinks because an outside alias cannot be proven safe, and it never emits raw URL, destination, fragment, secret, or outside-path values. For tiny explicit edits, inspect the target file and nearby docs directly.

The audit helper implements a bounded CommonMark/GFM subset for links rather than a full renderer. Its reference, inline, image, fence, and code-span behavior is guided by the [CommonMark specification](https://spec.commonmark.org/current/) and the [GFM specification](https://github.github.com/gfm/). The subset supports balanced and angle destinations plus one-line optional titles; first duplicate reference definitions win, nested image destinations are collected, and nested links are intentionally excluded.
...
- [references/guide-ctf-handoff.md](references/guide-ctf-handoff.md) - CTF status and multi-agent handoff documentation
