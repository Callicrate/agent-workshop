...
If operation class is unclear and the action is mutating, stop at a prepared payload, manifest, dry-run output, or ready-to-run plan instead of executing. Never treat a successful CLI return code as sufficient proof for production activation, destructive operations, or semantic output correctness.
